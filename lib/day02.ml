...
let get_total_score_from_round_choices_outcomes =
  List.fold_left (fun acc round_outcome ->
    acc
    + get_outcome_value round_outcome.outcome
    + get_shape_value round_outcome.round_choices.player
  ) 0
...
let part_1 round_strings =
  ...
  in
  let round_choices_outcomes =
    List.map get_round_choices_outcome_from_round_choices round_choices
  in
  let total_score =
    get_total_score_from_round_choices_outcomes round_choices_outcomes
  in
...
let get_player_choice_from_round_plan round_plan =
  ...
  | { opponent = Paper; outcome = Loss }
    -> Rock
  | { opponent = Rock; outcome = Win }
  | { opponent = Scissors; outcome = Loss }
  | { opponent = Paper; outcome = Draw }
    -> Paper
  | _ -> Scissors
...
let part_2 round_strings =
  ...
  in
  let round_choices_outcomes =
    List.map get_round_choices_outcome_from_round_plan round_plans
  in
  let total_score =
    get_total_score_from_round_choices_outcomes round_choices_outcomes
  in
